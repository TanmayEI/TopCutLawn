...
class SliderAdapter(imageUrl: ArrayList<String>) :
    ...
    override fun onBindViewHolder(viewHolder: SliderAdapter.SliderViewHolder?, position: Int) {
        ...
        if (viewHolder != null) {
           ...
           /* Picasso.with(requireContext()).load(response.body()!!
                .user_details.image).into(binding.ivProfile)
            val picasso= Picasso.get()
            picasso.load(sliderList.get(position))
                .placeholder(R.drawable.home_photo_icon).into(viewHolder.imageView)*/
